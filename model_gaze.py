...
class VideoDataset(torch.utils.data.Dataset):
    ...
    def _aggregate_gaze(self, indices, gaze_batch):
        """Aggregate gaze heatmaps over time between two consecutively sampled frames."""
        aggregated_gaze = [self._gaze_to_heatmap([[gaze_batch[indices[0]]]]).numpy().sum(axis=0)]
        for i in range(len(indices)-1):
            start_idx = indices[i]
            end_idx = indices[i+1]
            gaze_segment = gaze_batch[start_idx:end_idx]
            temp_gaze = []
            temp_gaze.append(self._gaze_to_heatmap([gaze_segment]).squeeze(0).squeeze(0).numpy())
            gaze_segment = np.array(temp_gaze).sum(axis=1)
            aggregated_gaze.append(gaze_segment)
        try: 
            aggregated_gaze = torch.Tensor(np.array(aggregated_gaze))
        except:
            pass
        return aggregated_gaze

    def _overlay_heatmap_on_frame(self,frames, heatmaps, alpha=0.6):
        """
        frame   : (H,W,3) uint8 RGB or BGR
        heatmap : (H,W) or (1,H,W) float heatmap
        ...
        for frame, heatmap in zip(frames, heatmaps):
            ...
            # Normalize heatmap between 0-255
            hm = heatmap - heatmap.min()
            if hm.max() > 0:
                hm = hm / hm.max()
            hm = (hm * 255).astype(np.uint8)

            heatmap_color = cv2.applyColorMap(hm, cv2.COLORMAP_HOT)

            # Build mask where heatmap > 0
            mask = hm > 0
            mask3 = np.stack([mask, mask, mask], axis=-1)

            # Replace only non-zero heatmap regions
            alpha = hm.astype(np.float32) / 255.0   # normalized (H,W) alpha mask

            # expand to 3 channels
            alpha3 = np.stack([alpha, alpha, alpha], axis=-1)

            overlay = (alpha3 * heatmap_color + (1 - alpha3) * frame).astype(np.uint8)

            frames_agg.append(overlay)
            # Blend images: heatmap on top of frame

        return np.array(frames_agg)


    def _gaze_to_heatmap(self, batch, shape=(1, 1, 448, 448), sigma=.005, amp=.5):
        """
        Converts a batch of gaze locations to heatmaps
        """
        ...
        for sample in batch:
            hm_seq = []
            for loc in sample:
                x = np.linspace(0, 1, shape[-2])
                y = np.linspace(0,1, shape[-1])
                x, y = np.meshgrid(x, y)
                mean_x, mean_y = np.asarray(loc)       
                
                gaussian = amp * np.exp(-(((x - mean_x)**2) / (2 * sigma**2) + ((y - mean_y)**2) / (2 * sigma**2)))

                # Normalize to 0-255 range for image saving
                gaussian_img = (gaussian * 255).astype(np.uint8)
        
                hm_seq.append(gaussian + hm)
            heatmaps.append(torch.Tensor(np.array(hm_seq)))
                
                
        return torch.Tensor(np.array(heatmaps))
    ...
    def __getitem__(self, idx):
        try:
            ...
            # ---- 1) Validate proficiency label ----
            prof = str(ann.get("proficiency_level", "")).strip().lower()
            mapping = {
                "novice": 0,
                "early expert": 1,
            ...
            if prof not in mapping:
                print(f"[SKIP] Invalid label '{prof}' at idx={idx}")
                return self[0] if idx != 0 else None

            label = mapping[prof]

            # ---- 2) Build gaze path, expand ~ safely ----
            gaze_dir = os.path.expanduser("~/stockage2/EgoExo4D_gaze/gaze_frame_label")
            gaze_path = os.path.join(gaze_dir, f"{ann['take_uid']}_frame_label.csv")

            if not os.path.exists(gaze_path):
                print(f"[SKIP] Missing gaze file: {gaze_path}")
                return self[0] if idx != 0 else None

            # Load gaze file safely
            try:
                gaze_df = pd.read_csv(gaze_path)
            except Exception as e:
                print(f"[SKIP] Failed to load gaze CSV at idx={idx}: {e}")
                return self[0] if idx != 0 else None

            if not {"x", "y"}.issubset(gaze_df.columns):
                print(f"[SKIP] Gaze CSV missing x/y columns at {gaze_path}")
                return self[0] if idx != 0 else None

            # ---- 3) Load videos ----
            video_paths = ann.get("video_paths", [])
            videos = []
            ...
            for cam_idx in self.camera_indices:
                ...
                raw_path = video_paths[cam_idx]
                selected_path = os.path.join(
                    self.video_root,
                    raw_path.replace("frame_aligned_videos", "frame_aligned_videos/downscaled/448")
                )

                if not os.path.exists(selected_path):
                    print(f"[SKIP] Missing video: {selected_path}")
                    continue

                try:
                    with av.open(selected_path) as container:
                        total_frames = container.streams.video[0].frames
                        indices = sample_frame_indices(self.num_frames, 4, gaze_df.shape[0])
                        video = read_video_pyav(container, indices)
                    videos.append(video)
                except Exception as e:
                    print(f"[SKIP] Failed to load video {selected_path}: {e}")
                    continue

            if len(videos) == 0:
                print(f"[SKIP] No videos found at idx={idx}")
                return self[0] if idx != 0 else None

            # ---- 4) Slice gaze safely ----
            if gaze_df.shape[0] <= indices[-1]:
                print(gaze_df.shape[0], indices[-1])
                print(f"[SKIP] Gaze shorter than required frames at idx={idx}")
                return self[0] if idx != 0 else None

            gaze_agg = self._aggregate_gaze(indices, gaze_df[["x", "y"]].to_numpy())
            
            frames_agg = self._overlay_heatmap_on_frame(videos[0], gaze_agg.numpy())  # First view
            
            gaze_np = gaze_df.iloc[indices][["x", "y"]].to_numpy()

            # ---- 5) Convert gaze → heatmap ----
            try:
                heatmap = self._gaze_to_heatmap(
                    [gaze_np],
                    shape=(self.num_frames, 1, 224, 224),
                    sigma=0.05,
                    amp=1,
                )
                heatmap = heatmap.squeeze(0).to(torch.float32)  # safer than float16 now
            except Exception as e:
                print(f"[SKIP] Heatmap conversion failed at idx={idx}: {e}")
                return self[0] if idx != 0 else None
            ...
            return {
                "videos": videos,
                "label": label,
                "gaze": heatmap.unsqueeze(1).to(torch.float16),
                "scenario": ann["video_paths"][0].split("/")[1]
            }

        except Exception as e:
            print(f"[SKIP] Unexpected failure at idx={idx}: {e}")
            return self[0] if idx != 0 else None
...
def collate_fn(examples, video_processor):
    ...
    if len(examples) == 0:
        return {} # Return None if no valid examples are found
    ...
    for ex in examples:
        ...
        gaze_data.append(ex["gaze"].numpy())
        scenarios.append(ex["scenario"])
    ...
    # batch["gaze"] = torch.tensor(np.array(gaze_data), dtype=torch.float16)
...
def main():
    ...
    parser.add_argument(
        "--output_dir",
        type=str,
        default="/home/tellong/Bureau/Code/Code/GazeSkill/SkillFormer/trained_models/SkillFormer_gaze",
    ...
    # Flags
    parser.add_argument("--do_train", action="store_true", default=True, help="Run training")
    parser.add_argument("--do_inference", action="store_true", default=True, help="Run inference")
    parser.add_argument("--model_path",  type=str, default="/home/tellong/Bureau/Code/Code/GazeSkill/SkillFormer/trained_models/SkillFormer_gaze/", help="Path to the model directory for inference")
